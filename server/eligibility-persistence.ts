...
function buildRecommendations(baseAnswers: BaseEligibilityAnswers) {
  return recommendProgramIds(baseAnswers);
}

function buildFollowUpQuestions(recommendations: RecommendationRecord[]) {
  const questionIds = new Set(recommendations.map((recommendation) => recommendation.programId));
  return getProgramFollowUpQuestions().filter((question) => question.programId && questionIds.has(question.programId));
}
...
export async function createEligibilitySessionRecord(baseAnswers: BaseEligibilityAnswers) {
  const recommendations = buildRecommendations(baseAnswers);
...
export async function determineEligibilitySessionRecord(
  sessionId: string,
  followUpAnswers: FollowUpAnswers,
) {
  if (isSupabaseConfigured()) {
    ...
    const patchResponse = await fetch(`${SUPABASE_URL}/rest/v1/eligibility_sessions?id=eq.${encodeURIComponent(sessionId)}`, {
      method: "PATCH",
      headers: {
        ...getHeaders(),
...
export function getEligibilityQuestions() {
  return seedDb.eligibility_question as EligibilityQuestionRecord[];
}
